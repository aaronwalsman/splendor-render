...
lighting_model_fragment_shader_old = '''
...
void main(){
    ...
    */
    ...
    vec3 camera_fragment_normal =
            vec3(inverse(camera_matrix) * vec4(fragment_normal_n,0));
    ...
    reflected_color = pow(
            reflected_color, vec3(reflect_gamma, reflect_gamma, reflect_gamma));
    ...
    for(int i = 0; i < num_point_lights; ++i){
        ...
        vec3 light_position = vec3(
                camera_matrix * vec4(point_light_data[2*i+1],1));
    ...
    for(int i = 0; i < num_direction_lights; ++i){
        ...
        vec3 light_direction = vec3(
                camera_matrix * vec4(direction_light_data[2*i+1],0));
